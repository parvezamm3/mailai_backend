...
from database import preferences_collection, users_collection, draft_messages_collection, inbox_conversations_collection
...
def get_dashboard_data():
    ...
    message_id = data.get('message_id').replace(
        '/', '-').replace('+', '_')  # For Outlook messages
    ...
    preferences = {
        'enable_importance_generation': user_pref_doc.get('enable_importance_generation', False),
        'enable_summarization_and_categorization': user_pref_doc.get('enable_summarization_and_categorization', False),
        'enable_reply_generation': user_pref_doc.get('enable_reply_generation', False)
    }
    # print(preferences)
    ...
    if not message_doc_exists:
        ...
        if provider == "outlook":
            ...
            prepare_conversation_thread_outlook(
                owner_email, conv_id, message_id)
        if provider == "gmail":
            # print(f"Provider {provider}")
...
@add_on_bp.route('/not_malicious', methods=['POST'])
def not_malicious():
    "Set the mail is not spam or have malicious contents"
    if not request.is_json:
        return jsonify({"status": "error", "message": "Request must be JSON"}), 400
    data = request.get_json()
    message_id = data.get('message_id').replace(
    ...
    inbox_conversations_collection.update_one(
        ...
        {
            '$set': {
                'messages.$[message].analysis.is_spam': False,
                'messages.$[message].analysis.is_malicious': False,
                'messages.$[message].analysis.completed': False,
            }
        },
        array_filters=[
    ...
    msg_doc = {
        ...
        'email_provider': data.get('platform')
    }
    conduct_analysis(data.get('user_email'), data.get('conv_id'), msg_doc)
...
def save_preferences():
    ...
    data = request.get_json()
    user_id = data.get('user_id')
    enable_importance_generation = data.get('enable_importance_generation')
    enable_summarization_and_categorization = data.get(
        'enable_summarization_and_categorization')
    enable_reply_generation = data.get('enable_reply_generation')

    if not user_id:
        return jsonify({"status": "error", "message": "User ID is required"}), 400

    update_data = {
        'enable_importance_generation': enable_importance_generation,
        'enable_summarization_and_categorization': enable_summarization_and_categorization,
        'enable_reply_generation': enable_reply_generation
    }
...
#     message_id = data.get('message_id').replace(
#         '/', '-').replace('+', '_')  # For Outlook messages
#     conv_id = data.get('conv_id').replace(
#         '/', '-').replace('+', '_')  # From Outlook context
...
def get_emails():
    ...
    for conv_doc in inbox_conversations_collection.find({}, {'_id': 0}):
        ...
        messages_out.sort(key=lambda m: m.get(
            'received_time') or '', reverse=False)
...
def get_email_analysis(conv_id, message_id, user_id):
    ...
    current_message_doc = inbox_conversations_collection.find_one(
        {'conv_id': conv_id, "email_address": user_id,
            'messages.message_id': message_id},
        {'_id': 0, 'messages.$': 1}
    )
    current_message = current_message_doc['messages'][0]
        ...
        if operator == "Outlook":
            ...
            if account_type == "licensed":
                access_token = get_outlook_access_token(
                    email_address, account_type, user_data)
                    ...
                    if folder_display_name == "緊急度高":
                        ...
                        while next_link:
                            ...
                            for conv in conversations[1:3]:
                                ...
                                # inbox_conversations_collection.update_one
                                prepare_conversation_thread_outlook(
                                    email_address, conversation_id, message_id)
...
#         if conv_id:
#             latest_message = get_latest_message_with_aggregation(
#                 conv_id, email_address)
#             latest_message_id = latest_message.get('message_id')
#             if latest_message_id:
#                 previous_emails_summary = latest_message.get(
#                     'previous_messages_summary', '')
#                 previous_email_sender = latest_message.get('sender', '')
#                 previous_email_receipients = latest_message.get(
#                     'receivers', [])
...
def validate_outgoing():
    ...
    try:
        ...
        response = call_gemini_api_structured_output(
        ...
        msg_doc = {
            ...
            'previous_emails_summary': previous_emails_summary,
            'previous_email_sender': previous_email_sender,
            'previous_email_receipients': previous_email_receipients,
            'previous_email_cc': previous_email_cc,
            'previous_email_bcc': previous_email_bcc,
            'analysis_result': json.dumps(response, ensure_ascii=False)
        }
        draft_messages_collection.insert_one(msg_doc)
        # print("Body :", body)
        # print(response)
        # response = {}
        ...
        #     'sensitive_data': {'comment': '個人情報や機密情報は含まれていません。', 'has_sensitive_data': True},
        #     'spelling_mistakes': {'comment': 'スペルミスはありません。', 'has_mistakes': False}}
        return jsonify({"status": "success", "message": "Email is ready to send.", "data": json.dumps(response, ensure_ascii=False)}), 200
...
def download_excel():
    ...
    all_conversations = list(
        inbox_conversations_collection.aggregate(pipeline))
    ...
    for doc in all_conversations:
        ...
        for message in messages:
            ...
            for attachment in attachments:
                worksheet.write('G'+str(msg_end),
                                attachment.get('name'), cell_format_center)
                worksheet.write(
                    'H'+str(msg_end), attachment.get('attachment_summary'), cell_format_left)
                msg_end += 1
            analysis = message.get('analysis', {})
            without_tz_str = message.get(
            ...
            if msg_end != msg_start:
                if analysis:
                    ...
                    worksheet.merge_range(
                        'J'+str(msg_start)+':J'+str(msg_end), score_and_reason, cell_format_left)
                    worksheet.merge_range(
                        'K'+str(msg_start)+':K'+str(msg_end), analysis.get('summary'), cell_format_left)
                    replies = analysis.get('replies', [])
                    reply_txt = ''
                    if replies:
                        reply_txt = f"簡潔 : {replies[0]}\n確認 : {replies[1]}\n丁寧 : {replies[2]}"
                    worksheet.merge_range(
                        'L'+str(msg_start)+':M'+str(msg_end), reply_txt, cell_format_left)
                    worksheet.merge_range(
                        'N'+str(msg_start)+':N'+str(msg_end), analysis.get('category'), cell_format_center)
                    worksheet.merge_range('I'+str(msg_start)+':I'+str(msg_end), message.get(
                        'previous_messages_summary', ''), cell_format_left)
                worksheet.merge_range(
                    'B'+str(msg_start)+':B'+str(msg_end), received_time, cell_format_left)
                worksheet.merge_range(
                    'C'+str(msg_start)+':C'+str(msg_end), message.get('subject'), cell_format_left)
                worksheet.merge_range(
                    'D'+str(msg_start)+':D'+str(msg_end), message.get('sender'), cell_format_left)
                worksheet.merge_range(
                    'E'+str(msg_start)+':F'+str(msg_end), message.get('body'), cell_format_left)
                ...
                if analysis:
                    ...
                    imp_desc = analysis.get('importance_description', '')
                    score_and_reason = f"{imp_score} : \n {imp_desc}"
                    worksheet.write('J'+str(msg_start),
                                    score_and_reason, cell_format_left)
                    worksheet.write('K'+str(msg_start),
                                    analysis.get('summary'), cell_format_left)
                    replies = analysis.get('replies', [])
                    reply_txt = ''
                    if replies:
                        reply_txt = f"簡潔 : {replies[0]}\n確認 : {replies[1]}\n丁寧 : {replies[2]}"
                    worksheet.merge_range(
                        'L'+str(msg_start)+':M'+str(msg_end), reply_txt, cell_format_left)
                    worksheet.write(
                        'N'+str(msg_start), analysis.get('category'), cell_format_center)
                    worksheet.write(
                        'I'+str(msg_start), message.get('previous_messages_summary', ''), cell_format_left)

                worksheet.write('B'+str(msg_start),
                                received_time, cell_format_left)
                worksheet.write('C'+str(msg_start),
                                message.get('subject'), cell_format_left)
                worksheet.write('D'+str(msg_start),
                                message.get('sender'), cell_format_left)
                worksheet.merge_range(
                    'E'+str(msg_start)+':F'+str(msg_end), message.get('body'), cell_format_left)
        ...
        worksheet.merge_range(
            'A'+str(start_row)+':A'+str(end_row), doc.get('subject'), cell_format_center)
        end_row += 1
        start_row = end_row
